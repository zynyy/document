...
注释标记一个或多个字符。主要用于东亚排版作为发音指南或包含其他注释

## 用法

```html
// 注意该标签存在一个子元素(除了rtc)则就应紧随一个 <rt> 元素标签
<ruby>
  子元素 or 文本内容<rt></rt>
</ruby>
```
...
1. edit: `<ins>`、`<del>`
1. textLevel: `<a>`、`<abbr>`、`<b>`、`<bdi>`、`<bdo>`、`<br>`、`<cite>`、`<code>`、`<data>`、`<dfn>`、`<em>`、`<i>`、`<kbd>`、`<mark>`、`<q>`、`<ruby>`、`<s>`、`<samp>`、`<small>`、`<span>`、`<strong>`、`<sub>`、`<sup>`、`<time>`、`<u>`、`<var>`、`<wbr>`、`<rtc>`、`<rt>`、`<rb>`
